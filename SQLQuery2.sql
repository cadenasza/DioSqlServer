...
SELECT Ano, COUNT(*) AS Quantidade FROM dbo.Filmes GROUP BY Ano ORDER BY COUNT(*) DESC;

SELECT PrimeiroNome, UltimoNome FROM dbo.Atores WHERE GENERO = 'M';
SELECT PrimeiroNome, UltimoNome, Genero FROM dbo.Atores WHERE GENERO = 'F' ORDER BY PrimeiroNome;
...
FROM dbo.Filmes f
INNER JOIN dbo.ElencoFilme e ON f.Id = e.IdFilme
INNER JOIN dbo.Atores a ON e.IdAtor = a.Id;